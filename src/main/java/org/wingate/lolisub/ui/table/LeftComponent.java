...
public class LeftComponent extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for(int i=n; i<view.getTable().getAssTableModel().getRowCount()-n; i++){
            ...
            AssEvent event = (AssEvent)view.getTable().getAssTableModel()
                    .getValueAt(i, 12);
    ...
    private int getCollapsedGroupLinesCount(AssEvent event, int found){
        if(event.getGroup() != null){
            found += event.getGroup().getItemsCount();
            for(AssEvent x : event.getGroup().getEvents()){
