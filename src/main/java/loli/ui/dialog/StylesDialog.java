...
public class StylesDialog extends JDialog {
    ...
    public StylesDialog(Frame owner) throws HColorException {
        super(owner);
        setModal(true);
        setResizable(false);
        setTitle(Sub.L.getString("dialog.styles"));
        setSize(900, 600);
        ...
        JButton btnOK = new JButton(Sub.L.getString("message.ok"));
        JButton btnCancel = new JButton(Sub.L.getString("message.cancel"));
        ...
        JTextField tfTextBGR = new JTextField(HColor.withScheme(Color.white, HColorScheme.BGR));
        JTextField tfTextHTML = new JTextField(HColor.withScheme(Color.white, HColorScheme.HTML));
        ...
        tfTextBGR.addActionListener((e)->{
            if(tfTextBGR.getText().matches("&H[A-Fa-f0-9]{6}&")){
                try {
                    HColor col = HColor.fromScheme(tfTextBGR.getText(), HColorScheme.BGR);
                    btnTextColor.setColor(col.getColor());
                    float alpha = slTextAlpha.getValue() / 255.0f;
                    selectedStyle.setTextColor(new HColor(col.getColor(), alpha));
                    preview.setSentenceSample(selectedStyle, previewText.getText());
                } catch (HColorException ex) {
        ...
        tfTextHTML.addActionListener((e)->{
            if(tfTextHTML.getText().matches("#[A-Fa-f0-9]{6}")){
                try {
                    HColor col = HColor.fromScheme(tfTextHTML.getText(), HColorScheme.HTML);
                    btnTextColor.setColor(col.getColor());
                    float alpha = slTextAlpha.getValue() / 255.0f;
                    selectedStyle.setTextColor(new HColor(col.getColor(), alpha));
                    preview.setSentenceSample(selectedStyle, previewText.getText());
        ...
        btnTextColor.addActionListener((e)->{
            try {
                ...
                tfTextBGR.setText(HColor.withScheme(c, HColorScheme.BGR));
                tfTextHTML.setText(HColor.withScheme(c, HColorScheme.HTML));
                float alpha = slTextAlpha.getValue() / 255.0f;
                selectedStyle.setTextColor(new HColor(c, alpha));
                preview.setSentenceSample(selectedStyle, previewText.getText());
            } catch (HColorException ex) {
                OnError.dialogErr(ex.getLocalizedMessage());
            }
        ...
        JTextField tfKaraBGR = new JTextField(HColor.withScheme(Color.yellow, HColorScheme.BGR));
        JTextField tfKaraHTML = new JTextField(HColor.withScheme(Color.yellow, HColorScheme.HTML));
        ...
        tfKaraBGR.addActionListener((e)->{
            if(tfKaraBGR.getText().matches("&H[A-Fa-f0-9]{6}&")){
                try {
                    HColor col = HColor.fromScheme(tfKaraBGR.getText(), HColorScheme.BGR);
                    btnKaraColor.setColor(col.getColor());
                    float alpha = slKaraAlpha.getValue() / 255.0f;
                    selectedStyle.setKaraokeColor(new HColor(col.getColor(), alpha));
                    preview.setSentenceSample(selectedStyle, previewText.getText());
                } catch (HColorException ex) {
                    OnError.dialogErr(ex.getLocalizedMessage());
                }
        ...
        tfKaraHTML.addActionListener((e)->{
            if(tfKaraHTML.getText().matches("#[A-Fa-f0-9]{6}")){
                try {
                    HColor col = HColor.fromScheme(tfKaraHTML.getText(), HColorScheme.HTML);
                    btnKaraColor.setColor(col.getColor());
                    float alpha = slKaraAlpha.getValue() / 255.0f;
                    selectedStyle.setKaraokeColor(new HColor(col.getColor(), alpha));
                    preview.setSentenceSample(selectedStyle, previewText.getText());
        ...
        btnKaraColor.addActionListener((e)->{
            try {
                ...
                tfKaraBGR.setText(HColor.withScheme(c, HColorScheme.BGR));
                tfKaraHTML.setText(HColor.withScheme(c, HColorScheme.HTML));
                float alpha = slKaraAlpha.getValue() / 255.0f;
                selectedStyle.setKaraokeColor(new HColor(c, alpha));
                preview.setSentenceSample(selectedStyle, previewText.getText());
            } catch (HColorException ex) {
                OnError.dialogErr(ex.getLocalizedMessage());
            }
        ...
        JTextField tfBordBGR = new JTextField(HColor.withScheme(Color.black, HColorScheme.BGR));
        JTextField tfBordHTML = new JTextField(HColor.withScheme(Color.black, HColorScheme.HTML));
        ...
        tfBordBGR.addActionListener((e)->{
            if(tfBordBGR.getText().matches("&H[A-Fa-f0-9]{6}&")){
                try {
                    HColor col = HColor.fromScheme(tfBordBGR.getText(), HColorScheme.BGR);
                    btnBordColor.setColor(col.getColor());
                    float alpha = slBordAlpha.getValue() / 255.0f;
                    selectedStyle.setOutlineColor(new HColor(col.getColor(), alpha));
                    preview.setSentenceSample(selectedStyle, previewText.getText());
                } catch (HColorException ex) {
                    OnError.dialogErr(ex.getLocalizedMessage());
                }
        ...
        tfBordHTML.addActionListener((e)->{
            if(tfBordHTML.getText().matches("#[A-Fa-f0-9]{6}")){
                try {
                    HColor col = HColor.fromScheme(tfBordHTML.getText(), HColorScheme.HTML);
                    btnBordColor.setColor(col.getColor());
                    float alpha = slBordAlpha.getValue() / 255.0f;
                    selectedStyle.setOutlineColor(new HColor(col.getColor(), alpha));
                    preview.setSentenceSample(selectedStyle, previewText.getText());
        ...
        btnBordColor.addActionListener((e)->{
            try {
                ...
                tfBordBGR.setText(HColor.withScheme(c, HColorScheme.BGR));
                tfBordHTML.setText(HColor.withScheme(c, HColorScheme.HTML));
                float alpha = slBordAlpha.getValue() / 255.0f;
                selectedStyle.setOutlineColor(new HColor(c, alpha));
                preview.setSentenceSample(selectedStyle, previewText.getText());
            } catch (HColorException ex) {
                OnError.dialogErr(ex.getLocalizedMessage());
            }
        ...
        JTextField tfShadBGR = new JTextField(HColor.withScheme(Color.black, HColorScheme.BGR));
        JTextField tfShadHTML = new JTextField(HColor.withScheme(Color.black, HColorScheme.HTML));
        ...
        tfShadBGR.addActionListener((e)->{
            if(tfShadBGR.getText().matches("&H[A-Fa-f0-9]{6}&")){
                try {
                    HColor col = HColor.fromScheme(tfShadBGR.getText(), HColorScheme.BGR);
                    btnShadColor.setColor(col.getColor());
                    float alpha = slShadAlpha.getValue() / 255.0f;
                    selectedStyle.setShadowColor(new HColor(col.getColor(), alpha));
                    preview.setSentenceSample(selectedStyle, previewText.getText());
                } catch (HColorException ex) {
                    OnError.dialogErr(ex.getLocalizedMessage());
                }
        ...
        tfShadHTML.addActionListener((e)->{
            if(tfShadHTML.getText().matches("#[A-Fa-f0-9]{6}")){
                try {
                    HColor col = HColor.fromScheme(tfShadHTML.getText(), HColorScheme.HTML);
                    btnTextColor.setColor(col.getColor());
                    float alpha = slShadAlpha.getValue() / 255.0f;
                    selectedStyle.setShadowColor(new HColor(col.getColor(), alpha));
                    preview.setSentenceSample(selectedStyle, previewText.getText());
        ...
        btnShadColor.addActionListener((e)->{
            try {
                ...
                tfShadBGR.setText(HColor.withScheme(c, HColorScheme.BGR));
                tfShadHTML.setText(HColor.withScheme(c, HColorScheme.HTML));
                float alpha = slShadAlpha.getValue() / 255.0f;
                selectedStyle.setShadowColor(new HColor(c, alpha));
                preview.setSentenceSample(selectedStyle, previewText.getText());
            } catch (HColorException ex) {
                OnError.dialogErr(ex.getLocalizedMessage());
            }
